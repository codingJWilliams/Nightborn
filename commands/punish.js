...
class PunishCommand extends Command {
  ...
  async exec(message, args) {
    ...
    if (args.punishment === "warn") {
      args.toPunish.send(buildWarnDM(args.rulebroken, args.toPunish, message.author, "You were warned in Nightborn Estate", "This is only a warning, however if you continue breaking rules further punishments may be given."));
      message.channel.send("**Warned :thumbsup: **");
      modlog.send(new Discord.RichEmbed()
        .setTitle(`${message.author.username}#${message.author.discriminator} warned ${args.toPunish.user.username}#${args.toPunish.user.discriminator}.`)
        .setDescription("Rule broken: " + args.rulebroken.toString() + ".")
        .setColor(0xFF0000))
      ...
      var unban_epoch = moment()
        .relativeTime('+' + args.lengthp)
        .valueOf();
      this.client.addJob(() => {
        args.toPunish.removeRole(mutedrole);
      }, unban_epoch)
      args.toPunish.addRole(mutedrole);
      args.toPunish.send(buildWarnDM(args.rulebroken, args.toPunish, message.author, "You were muted in Nightborn Estate", "You were muted for " + args.lengthp + "."));
      message.channel.send("**Muted :thumbsup: **");
      modlog.send(new Discord.RichEmbed()
        .setTitle(`${message.author.username}#${message.author.discriminator} muted ${args.toPunish.user.username}#${args.toPunish.user.discriminator}.`)
        .setDescription("Rule broken: " + args.rulebroken.toString() + ". Time muted: " + args.lengthp)
        .setColor(0xFF0000))
      ...
      args.toPunish.addRole(mutedrole);
      args.toPunish.send(buildWarnDM(args.rulebroken, args.toPunish, message.author, "You were jailed in Nightborn Estate", "You were jailed for " + args.lengthp + "."));
      message.channel.send("**Jailed :thumbsup: **");
      modlog.send(new Discord.RichEmbed()
        .setTitle(`${message.author.username}#${message.author.discriminator} jailed ${args.toPunish.user.username}#${args.toPunish.user.discriminator}.`)
        .setDescription("Rule broken: " + args.rulebroken.toString() + ". Time jailed: " + args.lengthp)
        .setColor(0xFF0000))
    }
    return;
  }
