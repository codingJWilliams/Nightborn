...
class ClanlessCommand extends Command {
  constructor() {
  ...
  exec(message) {
    return new Promise(function (resolve, reject) {
      ...
      clanless.map((m) => {
        q.push(() => {
          m.addRole(randomRolePls(), "ClanAssignBot")
            .catch(console.error)
          console.log("Assigned " + m.user.username + "#" + m.user.discriminator + " the clanrole")
          m.removeRole("320199117467025418", "Reasons :P")
            .catch(console.error)
...
function randomRolePls() {
  var clans = ["359029884552609793", "359031525192237067", "359037166128594965", "359032992531873794", "359030683693219840", "359036991993544714", "359031142349012992"];
  var d = clans[Math.floor(Math.random() * clans.length)];
  if (!d) d = "359035268260691979";
  return d
}
module.exports = ClanlessCommand;
