...
class StatusCommand extends Command {
    ...
    async exec(message) {
        ...
        if (require("../../helpers/getPlatform")
            ...
            var mem = await require("systeminformation")
                .mem()
            var disk = await require("systeminformation")
                .fsSize();
            shell.exec("pm2 jlist", function(code, stdout, stderr) {
                ...
                message.channel.send(new Discord.RichEmbed()
                    ...
                    .addField("CheckUser Bot", (checkuser ? (checkuser.pm2_env.status === "online" ? `:white_check_mark: Checkuser online, consuming ${Math.round(checkuser.monit.memory / (1000*1000))}mb of ram.` : ":skull_crossbones: Checkuser dead. Status: " + checkuser.pm2_env.status) : "Checkuser Dead"))
                    .addField("Economy Server", EcoSuccess ? `:white_check_mark: Contacted economy server in ${EcoTime}ms` : `:skull_crossbones: Could not connect to ecoserver.`))
            })
        }
    }
