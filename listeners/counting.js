...
class ReadyListener extends Listener {
    ...
    async exec(message) {
        ...
        if (message.content != (currentCount.count + 1) && message.author.id !== "193053876692189184") return message.delete();
        eco.award(message.author.id, 10);
        if (message.author.id !== "193053876692189184") await global.mongo.collection("currentCount").findOneAndUpdate({}, {$inc: { count: 1 }});
