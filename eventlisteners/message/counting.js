...
module.exports = async(message) => {
  ...
  var currentCount = await global.mongo.collection("currentCount")
    .findOne({});
  if (message.content != (currentCount.count + 1) || !canGo) return message.delete();
  eco.award(message.author.id, 10);
  await global.mongo.collection("currentCount")
    .findOneAndUpdate({}, {
      $inc: {
        count: 1
      }
    });
  if (!(await global.mongo.collection("counters")
      .findOne({
        id: message.author.id
      }))) {
    await global.mongo.collection("counters")
      .insertOne({
        counts: 0,
        id: message.author.id
      });
